...
def read_all_data(maps, root, normalize=True, noise=True):
    ...
    # Load estimated HI (input) maps   (rotations + flips)
    HI_esti = np.load('HI_esti_data.npy')

    # Load HI and DM (True) maps       (rotations + flips)
...
####################### Histogram ########################
#Find mean values of predictions based on test set
predictions = np.zeros((100, 32, 32))
mean = np.zeros(100)

for i in range(100):
    predictions[i] = model(input[i].unsqueeze_(0)).detach().numpy()
    mean[i] = np.mean(predictions[i])
    
#Find mean values HI_images in test set
mean_HI = np.zeros(100)

for i in range(100):
    mean_HI[i] = np.mean(HI_True[i].detach().numpy())
    
plt.hist(mean, alpha = 0.5, label = 'Predictions')
plt.hist(mean_HI, alpha = 0.5, label = 'True Images')
